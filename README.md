...
The utility parameterizes values by replacing the values in the list that Kysely passes to the database for parameterizing the query; values are not inserted into the query. For this reason, the package should be as safe as providing values to Kysely directly.
